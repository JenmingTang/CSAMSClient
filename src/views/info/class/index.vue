...
import {
  deleteInfoClassById,
  deleteInfoClassListByIdList,
  deleteInfoDepartmentListByIdList,
  selectInfoClassList,
  selectInfoClassListByParams
...
const {
  ...
  apiFn: selectInfoClassListByParams,
  apiParams: {
    current: 1,
    size: 10,
    ...
    },
    {
      ...
      render: row => (
        <div class="flex-center gap-8px">
          {hasAuth('BUTTON_EDIT_CLASS') ? (
            <NButton type="primary" ghost size="small" onClick={() => edit(row.id)}>
              {$t('common.edit')}
            </NButton>
          ) : null}
          {hasAuth('BUTTON_DELETE_CLASS') && (
...
async function handleBatchDelete() {
  ...
  await deleteInfoClassListByIdList(checkedRowKeys.value);
  onBatchDeleted();
}
...
async function exportExcel(isAllData: boolean, e: any) {
  ...
  if (isAllData) {
    const resp = await selectInfoClassListByParams({
      size: 999999
    });
  ...
  utils.book_append_sheet(workBook, workSheet, '班级列表');

  writeFile(workBook, '班级数据.xlsx');
} // tang
    ...
    <NCard title="班级列表" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
      <template #header-extra>
        <NSpace align="end" wrap justify="end" class="lt-sm:w-200px">
          <NButton v-if="hasAuth('BUTTON_EXCEL_CLASS')" size="small" ghost type="primary" @click="exportExcel(false)">
            <template #icon>
          ...
          <NButton
            v-if="hasAuth('BUTTON_EXCEL_CLASS_ALL')"
            size="small"
            ghost
            type="primary"
          ...
          <TableHeaderOperation
            ...
            :loading="loading"
            show-add-button-str="BUTTON_ADD_CLASS"
            show-batch-delete-button-str="BUTTON_BATCH_DELETE_CLASS"
            @add="handleAdd"
