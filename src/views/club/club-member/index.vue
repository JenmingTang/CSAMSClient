...
import {
  ...
  deleteClubMemberListByIdList,
  selectClubMemberList,
  selectClubMemberListByParams
...
const {
  ...
  apiFn: selectClubMemberListByParams,
  apiParams: {
    current: 1,
    size: 10,
    createTime: null,
    ...
    },
    {
      ...
      render: row => (
        <div class="flex-center gap-8px">
          {hasAuth('BUTTON_EDIT_CLUB_MEMBER') ? (
            <NButton type="primary" ghost size="small" onClick={() => edit(row.id)}>
              {$t('common.edit')}
            </NButton>
          ) : null}
          {hasAuth('BUTTON_DELETE_CLUB_MEMBER') && (
...
async function handleBatchDelete() {
  ...
  await deleteClubMemberListByIdList(checkedRowKeys.value);
  onBatchDeleted();
}
...
async function exportExcel(isAllData: boolean, e: any) {
  ...
  if (isAllData) {
    const resp = await selectClubMemberListByParams({
      size: 999999
    });
  ...
  utils.book_append_sheet(workBook, workSheet, '社团成员列表');

  writeFile(workBook, '社团成员数据.xlsx');
} // tang
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <NCard title="社团成员列表" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
      <template #header-extra>
        <NSpace align="end" wrap justify="end" class="lt-sm:w-200px">
          <NButton
            v-if="hasAuth('BUTTON_EXCEL_CLUB_MEMBER')"
            size="small"
            ghost
            type="primary"
          ...
          <NButton
            v-if="hasAuth('BUTTON_EXCEL_CLUB_MEMBER_ALL')"
            size="small"
            ghost
            type="primary"
          ...
          <TableHeaderOperation
            ...
            :loading="loading"
            show-add-button-str="BUTTON_ADD_CLUB_MEMBER"
            show-batch-delete-button-str="BUTTON_BATCH_DELETE_CLUB_MEMBER"
            @add="handleAdd"
