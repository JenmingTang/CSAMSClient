      ...
      <NCollapseItem :title="$t('common.search')" name="role-search">
        <NForm :model="model" label-placement="left" :label-width="80">
          <NGrid responsive="screen" item-responsive>
            <NFormItemGi span="24 s:12 m:6" label="用户名称" path="realName" class="pr-24px">
              <NInput v-model:value="model.userName" />
            </NFormItemGi>

            <NFormItemGi span="24 s:12 m:6" label="所属活动" path="realName" class="pr-24px">
              <NInput v-model:value="model.activityName" />
            </NFormItemGi>

            <NFormItemGi span="24 s:12 m:6" label="加入时间（前后共一小时）" path="status" class="pr-24px">
...
-->
            <NFormItemGi span="24 s:12 m:6">
              <NSpace class="w-full" justify="end">
                ...
                <NButton v-if="hasAuth('BUTTON_SEARCH_ACTIVITY_MEMBER')" type="primary" ghost @click="search">
                  <template #icon>
