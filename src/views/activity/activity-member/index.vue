...
import {
  deleteActivityMemberById,
  deleteActivityMemberListByIdList,
  selectActivityMemberListByParams
...
const {
  ...
  apiFn: selectActivityMemberListByParams,
  apiParams: {
    current: 1,
    size: 10,
    approvalStatus: null,
...

    {
      ...
      render: row => (
        <div class="flex-center gap-8px">
          {hasAuth('BUTTON_EDIT_ACTIVITY_MEMBER') ? (
            <NButton type="primary" ghost size="small" onClick={() => edit(row.id)}>
              {$t('common.edit')}
            </NButton>
          ) : null}
          {hasAuth('BUTTON_DELETE_ACTIVITY_MEMBER') && (
...
async function handleBatchDelete() {
  ...
  await deleteActivityMemberListByIdList(checkedRowKeys.value);
  onBatchDeleted();
}
...
async function handleDelete(id: number) {
  // request
  console.log(id);
  await deleteActivityMemberById(id);
  onDeleted();
}
...
async function exportExcel(isAllData: boolean, e: any) {
  ...
  if (isAllData) {
    const resp = await selectActivityMemberListByParams({
      size: 999999
    });
  ...
  utils.book_append_sheet(workBook, workSheet, '活动成员列表');

  writeFile(workBook, '活动成员数据.xlsx');
} // tang
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <NCard title="活动成员列表" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
      <template #header-extra>
        <NSpace align="end" wrap justify="end" class="lt-sm:w-200px">
          <NButton
            v-if="hasAuth('BUTTON_EXCEL_ACTIVITY_MEMBER')"
            size="small"
            ghost
            type="primary"
          ...
          <NButton
            v-if="hasAuth('BUTTON_EXCEL_ACTIVITY_MEMBER_ALL')"
            size="small"
            ghost
            type="primary"
          ...
          <TableHeaderOperation
            ...
            :loading="loading"
            show-add-button-str="BUTTON_ADD_ACTIVITY_MEMBER"
            show-batch-delete-button-str="BUTTON_BATCH_DELETE_ACTIVITY_MEMBER"
            @add="handleAdd"
