...
import {
  deleteActivityLocationById,
  deleteActivityLocationListByIdList,
  selectActivityLocationListByParams
...
const {
  ...
  apiFn: selectActivityLocationListByParams,
  apiParams: {
    current: 1,
    size: 10,
    deleted: null,
    ...
    },
    {
      ...
      render: row => (
        <div class="flex-center gap-8px">
          {hasAuth('BUTTON_EDIT_ACTIVITY_LOCATION') ? (
            <NButton type="primary" ghost size="small" onClick={() => edit(row.id)}>
              {$t('common.edit')}
            </NButton>
          ) : null}
          {hasAuth('BUTTON_DELETE_ACTIVITY_LOCATION') && (
...
async function handleBatchDelete() {
  ...
  await deleteActivityLocationListByIdList(checkedRowKeys.value);
  onBatchDeleted();
}
...
async function handleDelete(id: number) {
  // request
  console.log(id);
  await deleteActivityLocationById(id);
  onDeleted();
}
...
async function exportExcel(isAllData: boolean, e: any) {
  ...
  if (isAllData) {
    const resp = await selectActivityLocationListByParams({
      size: 999999
    });
  ...
  utils.book_append_sheet(workBook, workSheet, '活动地点列表');

  writeFile(workBook, '活动地点数据.xlsx');
} // tang
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <NCard title="活动地点列表" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
      <template #header-extra>
        <NSpace align="end" wrap justify="end" class="lt-sm:w-200px">
          <NButton
            v-if="hasAuth('BUTTON_EXCEL_ACTIVITY_LOCATION')"
            size="small"
            ghost
            type="primary"
          ...
          <NButton
            v-if="hasAuth('BUTTON_EXCEL_ACTIVITY_LOCATION_ALL')"
            size="small"
            ghost
            type="primary"
          ...
          <TableHeaderOperation
            ...
            :loading="loading"
            show-add-button-str="BUTTON_ADD_ACTIVITY_LOCATION"
            show-batch-delete-button-str="BUTTON_BATCH_DELETE_ACTIVITY_LOCATION"
            @add="handleAdd"
