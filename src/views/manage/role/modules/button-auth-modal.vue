...
import { selectPermissionIdListByRoleId, selectPermissionList, updateRolePermissions } from '@/service/api/csams/sys';
defineOptions({
...
async function getAllButtons() {
  const { data } = await selectPermissionList();
  data.forEach(item => {
    tree.value.push({
      id: item.id,
...
async function getChecks() {
  ...
  const { data } = await selectPermissionIdListByRoleId(props.roleId);
  checks.value = data;
}
...
async function handleSubmit() {
  ...
  // request
  await updateRolePermissions(props.roleId, checks.value);
